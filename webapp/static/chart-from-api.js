...
function populateStateSelector() {
    ...
    .then(function(states) {
        let stateSelector = document.getElementById('state-select');
        if (stateSelector) {
            // Populate it with states from the API
            let stateSelectorBody = '<option value="US">United States</option>\n';
...
function onStateSelectorChanged() {
    let stateSelector = document.getElementById('state-select');
    if (stateSelector) {
        let stateName = stateSelector.options[stateSelector.selectedIndex].text
        let stateAbbreviation = stateSelector.value;
...
function createStateChart(stateName, stateAbbreviation) {
    // Set the title
    let stateTitle = document.getElementById('state-new-cases-title');
    if (stateTitle) {
    ...
    .then(function(days) {
        ...
        for (let k = 0; k < days.length; k++) {
            ...
            let date = '' + days[days.length - k -1].date;
